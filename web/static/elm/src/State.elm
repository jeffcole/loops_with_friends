...
initialState flags =
    let
        ( socket, socketCmds ) =
            Socket.joinChannel flags.host flags.topic
    in
        ( { userId = ""
    ...
    case msg of
        ...
        RetryJoin newTopic ->
            let
                ...
                host =
                    Socket.pathToHost path

                ( socket, socketCmds ) =
                    Socket.joinChannel host newTopic
            in
                ( { model | socket = socket }
                , Cmd.map SocketMsg socketCmds
                )
        ...
        Play ->
            let
                playerUser =
                    Helpers.playerUser model

                ( user, cmds ) =
                    User.State.queueLoop playerUser

                users =
                    User.Helpers.replace user model.users
            in
                ( { model | users = users }
        ...
        Stop ->
            let
                ( _, cmds ) =
                    Helpers.playerUser model |> User.State.stopLoop
            in
        ...
        UserPlayed json ->
            let
                userId =
                    Socket.decodeUserId json

                user =
                    User.Helpers.getUser userId model.users

                ( newUser, cmds ) =
                    User.State.queueLoop user

                users =
                    User.Helpers.replace newUser model.users
            in
                ( { model | users = users }
        ...
        UserMsg userId userMsg ->
            let
                ...
                ( newUser, userCmds, outMsg ) =
                    User.State.update userMsg user
        ...
        MeasureStart time ->
            let
                ( users, cmds ) =
                    User.State.playQueuedLoops model.users
            in
                ( { model | users = users }
        ...
        SocketMsg socketMsg ->
            let
                ( socket, socketCmds ) =
                    Phoenix.Socket.update socketMsg model.socket
            in
                ( { model | socket = socket }
                , Cmd.map SocketMsg socketCmds
                )
...
measureSubscriptions model =
    if User.Helpers.anyLoopsQueued model.users then
        Time.every (4.799625 * Time.second) MeasureStart
    else
        Sub.none
...
pushIfPlayerUser outMsg userId model =
    if userId == model.userId then
        Socket.pushUserMsg outMsg userId model.socket
    else
